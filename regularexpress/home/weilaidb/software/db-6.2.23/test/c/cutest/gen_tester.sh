#!/usr/bin/env bash
# Auto generate single AllTests file for CuTest.
# Searches through all *.c files in the current directory.
# Prints to stdout.
# Author: Asim Jalis
# Date: 01/08/2003
# Edited 2010 Alex G to match DB tree layout.
if test $# -eq 0 ; then FILES=../suites
'
NEXT_STRING=`grep -h "int Test" $ | sed -e 's/ {/;/' -e 's/^/extern /'`
echo "$NEXT_STRING
"
# Want to turn TestEnvConfig.c into:
# Function name RunEnvConfigTests
# Suite name TestEnvConfig
for f in `ls $`; do
SUITE_NAME=`basename $f .c`
RUNNER_NAME=`echo $SUITE_NAME | sed -e 's/Test\(.*\)/Run\1Tests/'`
TEST_STR=`grep "$SuiteSetup" $f`
if [ "$TEST_STR"x != "x" ]; then
SETUP_FN=$SuiteSetup
else
SETUP_FN="NULL"
fi
TEST_STR=`grep "$SuiteTeardown" $f`
if [ "$TEST_STR"x != "x" ]; then
TEARDOWN_FN=$SuiteTeardown
else
TEARDOWN_FN="NULL"
fi
echo \
"int $RUNNER_NAME(CuString *output)
{
CuSuite *suite = CuSuiteNew(\"$SUITE_NAME\",
$SETUP_FN, $TEARDOWN_FN);
int count;
"
TEST_STR=`grep "$TestSetup" $f`
if [ "$TEST_STR"x != "x" ]; then
SETUP_FN=$TestSetup
else
SETUP_FN="NULL"
fi
TEST_STR=`grep "$TestTeardown" $f`
if [ "$TEST_STR"x != "x" ]; then
TEARDOWN_FN=$TestTeardown
else
TEARDOWN_FN="NULL"
fi
NEXT_STRING=`grep -h "int Test" $f | sed -e '/[Suite|Test]Setup/d' -e '/[Suite|Test]Teardown/d' -e 's/^int /\tSUITE_ADD_TEST(suite, /' -e "s/(CuTest \*ct)
'
done
echo \
'TestSuite g_suites[] = ;
'
