...
test_must_fail git rev-parse --verify 000000000^ 2>actual &&
...
test $(git rev-parse $commit^) = $(git rev-parse 0000000000cdc)
'
...
git rev-parse --verify 0000000000e4f^ &&
# ambiguous at the object name level, but there is only one
# such commit (the others are tree and blob)
git rev-parse --verify 000000000^ &&
# likewise
...
# 0000000000 before ^ name many different objects, but
# that only two (HEAD and v1.0.0 tag) can be peeled to commit,
# and that peeling them down to commit yield the same commit
# without ambiguity.
git rev-parse --verify 0000000000^ &&
# likewise
...
# Likewise for tree-ish.  HEAD, v1.0.0 and HEAD^ share
...
test_must_fail git rev-parse --verify 00000000^ &&
# likewise
