...
git rev-list --pretty=raw $ | grep ^tree | uniq > a &&
...
svn mkdir -m "mkdir bar" "$2/bar" &&
git config --unset svn-remote.svn.fetch \
"^bar:refs/$$" &&
test_must_fail git svn init "$2/bar"
'
test_expect_success \
