...
test_must_fail git rev-parse --verify delta@ &&
git commit -m Delta &&
git rev-parse --verify delta@
'
test_expect_success '--orphan does not make reflog when core.logAllRefUpdates = false' '
git checkout master &&
git config core.logAllRefUpdates false &&
git checkout --orphan epsilon &&
test_must_fail git rev-parse --verify epsilon@ &&
...
test_must_fail git rev-parse --verify zeta@ &&
git commit -m Zeta &&
git rev-parse --verify zeta@
'
test_expect_success 'giving up --orphan not committed when -l and core.logAllRefUpdates = false deletes reflog' '
git checkout master &&
git checkout -l --orphan eta &&
test_must_fail git rev-parse --verify eta@ &&
git checkout master &&
test_must_fail git rev-parse --verify eta@
'
