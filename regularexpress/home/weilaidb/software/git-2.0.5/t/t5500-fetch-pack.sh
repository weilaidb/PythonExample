...
git clone --single-branch --branch A "file:
test_must_fail git --git-dir=branch-a/.git rev-parse origin/B
'
test_expect_success 'clone shallow depth 1' '
git clone --no-single-branch --depth 1 "file:
...
git clone --no-single-branch --depth 2 "file:
'
...
git clone --depth 1 --branch A "file:
'
test_expect_success 'clone shallow object count' '
...
git clone --depth 1 --branch=TAGB1 "file:
cat >taglist.expected <<\EOF &&
TAGB1
TAGB2
EOF
...
(
git clone --bare --branch B --single-branch "file:
git clone --depth 1 "file:
cd no-reflog &&
...
test_expect_success "fetch-pack --diag-url $p:
check_prot_host_path $p:
'
# "/~" -> "~" conversion
test_expect_success "fetch-pack --diag-url $p:
check_prot_host_path $p:
...
test_expect_success MINGW 'fetch-pack --diag-url file:
check_prot_path file:
'
