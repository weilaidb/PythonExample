...
test_must_fail git stash apply stash@ &&
echo 1 >expect &&
...
echo bar > file &&
git stash &&
test_must_fail git stash drop $(git rev-parse stash@) &&
git stash pop &&
test bar = "$(cat file)" &&
git reset --hard HEAD
'
...
echo bar > file &&
git stash &&
test_must_fail git stash pop $(git rev-parse stash@) &&
git stash pop &&
test bar = "$(cat file)" &&
git reset --hard HEAD
'
...
git stash drop
'
test_expect_success 'invalid ref of the form stash@, n >= N' '
git stash clear &&
...
test_must_fail git stash apply stash@ &&
test_must_fail git stash show stash@ &&
test_must_fail git stash branch tmp stash@ &&
git stash drop
'
...
echo bar >file &&
git stash &&
test_must_fail git stash branch master stash@ &&
git rev-parse stash@ --
'
