...
added=$(printf "added (hinzugef$gt) foo")
...
changed=$(printf "changed (ge$ndert) foo")
...
test_tick &&
C=$(GIT_AUTHOR_EMAIL= git commit-tree HEAD^ </dev/null) &&
...
git commit -m "dummy" --allow-empty &&
git filter-branch --msg-filter "sed -e s/dummy
