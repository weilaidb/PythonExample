...
test_must_fail git p4 clone --git-dir=xx
'
test_expect_success 'clone --branch should checkout master' '
git p4 clone --branch=refs/remotes/p4/sb --dest="$git"
...
cd "$git" &&
git init &&
git p4 sync --branch=refs/remotes/p4/b1
...
printf "1\n3\n" >cf &&
git p4 clone --changesfile="$TRASH_DIRECTORY/cf" --dest="$git"
test_when_finished cleanup_git &&
(
cd "$git" &&
...
test_when_finished "rm cf" &&
printf "1\n3\n" >cf &&
test_must_fail git p4 clone --changesfile="$TRASH_DIRECTORY/cf" --dest="$git"
'
# imports both master and p4/master in refs/heads
# requires --import-local on sync to find p4 refs/heads
# does not update master on sync, just p4/master
test_expect_success 'clone/sync --import-local' '
git p4 clone --import-local --dest="$git"
test_when_finished cleanup_git &&
(
cd "$git" &&
...
git p4 clone --dest="$git" --max-changes 2
test_when_finished cleanup_git &&
(
cd "$git" &&
git log --oneline refs/heads/master >lines &&
...
git p4 clone --dest="$git" --keep-path
test_when_finished cleanup_git &&
(
cd "$git" &&
...
cli="$cli2" &&
client_view "
git p4 clone --dest="$git" --use-client-spec
(
cd "$git" &&
