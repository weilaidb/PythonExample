...
cd "$git" &&
test_commit head &&
git p4 sync --branch=refs/remotes/p4/depot
...
export PATH &&
test_expect_code 1 git p4 clone --dest="$git"
) &&
cat errs &&
...
p4change=$(p4 -G changes -m 1
p4time=$(p4 -G changes -m 1
sleep 3 &&
git p4 clone --dest="$git"
test_when_finished cleanup_git &&
(
