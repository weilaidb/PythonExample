...
test_when_finished cleanup_git &&
git p4 clone --dest="$git" --detect-branches
(
cd "$git" &&
git log --oneline --graph --decorate --all &&
# 4 main commits
...
(
cd "$cli" &&
changelist=$(p4 changes -m1
changelist=$(($changelist - 5)) &&
p4 integrate
p4 submit -d "Integrate branch4 from branch1@$" &&
changelist=$(($changelist + 2)) &&
p4 integrate
p4 submit -d "Integrate branch5 from branch1@$"
)
'
