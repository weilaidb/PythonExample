...
'
cat >expect <<'EOF'
Reflog: HEAD@ (C O Mitter <committer@example.com>)
...
'
cat >expect <<'EOF'
e46513e HEAD@: commit (initial): one
EOF
...
Reflog message: commit (initial): one
EOF
test_expect_success 'using @ syntax shows reflog date (multiline)' '
git log -g -1 HEAD@ >tmp &&
grep ^Reflog <tmp >actual &&
test_cmp expect actual
'
cat >expect <<'EOF'
e46513e HEAD@: commit (initial): one
EOF
test_expect_success 'using @ syntax shows reflog date (oneline)' '
git log -g -1 --oneline HEAD@ >actual &&
test_cmp expect actual
'
cat >expect <<'EOF'
HEAD@
EOF
test_expect_success 'using @ syntax shows reflog date (format=%gd)' '
git log -g -1 --format=%gd HEAD@ >actual &&
test_cmp expect actual
'
cat >expect <<'EOF'
Reflog: HEAD@ (C O Mitter <committer@example.com>)
...
HEAD@
EOF
test_expect_success '--date magic does not override explicit @ syntax' '
git log -g -1 --format=%gd --date=raw HEAD@ >actual &&
test_cmp expect actual
'
: >expect
