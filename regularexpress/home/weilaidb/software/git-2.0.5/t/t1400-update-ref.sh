...
test_expect_success \
'Query "master@" (before history)' \
'rm -f o e &&
git rev-parse --verify "master@" >o 2>e &&
test '"$C"' = $(cat o) &&
...
test_expect_success \
'Query "master@" (1 second before history)' \
'rm -f o e &&
git rev-parse --verify "master@" >o 2>e &&
test '"$C"' = $(cat o) &&
test "warning: Log for '\''master'\'' only goes back to $ed." = "$(cat e)"'
test_expect_success \
'Query "master@" (exactly history start)' \
'rm -f o e &&
git rev-parse --verify "master@" >o 2>e &&
test '"$C"' = $(cat o) &&
test "" = "$(cat e)"'
test_expect_success \
'Query "master@" (first non-creation change)' \
'rm -f o e &&
git rev-parse --verify "master@" >o 2>e &&
test '"$A"' = $(cat o) &&
test "" = "$(cat e)"'
test_expect_success \
'Query "master@" (middle of history with gap)' \
'rm -f o e &&
git rev-parse --verify "master@" >o 2>e &&
test '"$B"' = $(cat o) &&
test "warning: Log .git/logs/'"$m has gap after $gd"'." = "$(cat e)"'
test_expect_success \
'Query "master@" (middle of history)' \
'rm -f o e &&
git rev-parse --verify "master@" >o 2>e &&
test '"$Z"' = $(cat o) &&
test "" = "$(cat e)"'
test_expect_success \
'Query "master@" (exact end of history)' \
'rm -f o e &&
git rev-parse --verify "master@" >o 2>e &&
test '"$E"' = $(cat o) &&
test "" = "$(cat e)"'
test_expect_success \
'Query "master@" (past end of history)' \
'rm -f o e &&
git rev-parse --verify "master@" >o 2>e &&
test '"$D"' = $(cat o) &&
...
test_expect_success \
'git cat-file blob master@:F (expect TEST)' \
'test TEST = $(git cat-file blob "master@:F")'
test_expect_success \
'git cat-file blob master@:F (expect OTHER)' \
'test OTHER = $(git cat-file blob "master@:F")'
a=refs/heads/a
