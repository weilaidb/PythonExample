...
test_expect_success \
'A: verify tree' \
'git cat-file -p master^ | sed "s/ [0-9a-f]*	/ /" >actual &&
test_cmp expect actual'
...
test $old_branch = `git rev-parse --verify branch@`'
###
### series H
###
test_tick
...
'git fast-import <input &&
test `git rev-parse N2^` = `git rev-parse N3^`'
test_expect_success \
...
EOF
root=$(git rev-parse refs/heads/branch^0^) &&
cat >input <<-INPUT_END &&
...
git log --reverse --pretty=oneline O3 | sed s/^.*z
test_cmp expect actual'
cat >input <<INPUT_END
...
'git cat-file -p refs/notes/foobar~2^ | sed "s/ [0-9a-f]*	/ /" >actual &&
test_cmp expect actual'
...
'git cat-file -p refs/notes/foobar^^ | sed "s/ [0-9a-f]*	/ /" >actual &&
test_cmp expect actual'
...
'git cat-file -p refs/notes/foobar2^ | sed "s/ [0-9a-f]*	/ /" >actual &&
test_cmp expect actual'
