...
cd prune-namespace &&
git fetch --prune origin refs/heads/ahead:refs/remotes/origin/prhead:refs/remotes/origin/pr/* &&
...
head -n 2 "$D"/bundle1 | grep "^-[0-9a-f]\ " &&
head -n 3 "$D"/bundle1 | grep "^[0-9a-f]\ " &&
...
git config branch.master.merge "$_not" &&
git update-ref -d FETCH_HEAD &&
git fetch one &&
test $one_head = "$(git rev-parse --verify FETCH_HEAD)" &&
test $this_head = "$(git rev-parse --verify HEAD)"
...
test_bundle_object_count .git/objects/pack/pack-$.pack 3
'
