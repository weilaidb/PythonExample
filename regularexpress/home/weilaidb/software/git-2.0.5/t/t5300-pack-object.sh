...
tree=`git write-tree` &&
commit=`git commit-tree $tree </dev/null` &&  >obj-list &&  >expect'
test_expect_success \
'pack without delta' \
...
export GIT_OBJECT_DIRECTORY &&
git init &&
git unpack-objects -n <test-3-$.pack &&
git unpack-objects <test-3-$.pack'
unset GIT_OBJECT_DIRECTORY
cd "$TRASH/.git2"
test_expect_success \
...
export GIT_OBJECT_DIRECTORY &&
git init &&
cp test-1-$.pack test-1-$.idx .git2/objects/pack &&  >current &&
cmp expect current'
test_expect_success \
...
cp test-2-$.pack test-2-$.idx .git2/objects/pack &&  >current &&
cmp expect current'
test_expect_success \
...
export GIT_OBJECT_DIRECTORY &&
rm -f .git2/objects/pack/test-* &&
cp test-3-$.pack test-3-$.idx .git2/objects/pack &&  >current &&
cmp expect current'
unset GIT_OBJECT_DIRECTORY
...
rm -fr $GOP &&
git index-pack --stdin --keep=test <../test-3-$.pack &&
test -f $GOP/pack-$.pack &&
cmp $GOP/pack-$.pack ../test-3-$.pack &&
test -f $GOP/pack-$.idx &&
cmp $GOP/pack-$.idx ../test-3-$.idx &&
test -f $GOP/pack-$.keep
)
'
test_expect_success \
'verify pack' \
'git verify-pack	test-1-$.idx \
test-2-$.idx \
test-3-$.idx'
test_expect_success \
'verify pack -v' \
'git verify-pack -v	test-1-$.idx \
test-2-$.idx \
test-3-$.idx'
test_expect_success \
'verify-pack catches mismatched .idx and .pack files' \
'cat test-1-$.idx >test-3.idx &&
cat test-2-$.pack >test-3.pack &&
...
cmp tmp.idx test-1-$.idx &&
git index-pack test-3.pack &&
cmp test-3.idx test-1-$.idx &&
cat test-2-$.pack >test-3.pack &&
git index-pack -o tmp.idx test-2-$.pack &&
cmp tmp.idx test-2-$.idx &&
git index-pack test-3.pack &&
cmp test-3.idx test-2-$.idx &&
cat test-3-$.pack >test-3.pack &&
git index-pack -o tmp.idx test-3-$.pack &&
cmp tmp.idx test-3-$.idx &&
git index-pack test-3.pack &&
cmp test-3.idx test-3-$.idx &&
:'
