...
Requires:       git = %-%
Requires:       perl(:MODULE_COMPAT_%(eval "`% -V:version`"; echo $version))
BuildRequires:  perl(Error)
BuildRequires:  perl(ExtUtils::MakeMaker)
%description -n perl-Git
Perl interface to Git
%define path_settings ETC_GITCONFIG=/etc/gitconfig prefix=% mandir=% htmldir=%/%-%
%
%prep
%setup -q
%build
...
test ! -d $RPM_BUILD_ROOT% || rm -fr $RPM_BUILD_ROOT%
find $RPM_BUILD_ROOT -type f -name .packlist -exec rm -f  ';'
find $RPM_BUILD_ROOT -type f -name '*.bs' -empty -exec rm -f  ';'
find $RPM_BUILD_ROOT -type f -name perllocal.pod -exec rm -f  ';'
(find $RPM_BUILD_ROOT% -type f | grep -vE "archimport|svn|cvs|email|gitk|git-gui|git-citool" | sed -e s@^$RPM_BUILD_ROOT@@)               > bin-man-doc-files
(find $RPM_BUILD_ROOT%/git-core -type f | grep -vE "archimport|svn|cvs|email|gitk|git-gui|git-citool" | sed -e s@^$RPM_BUILD_ROOT@@)               >> bin-man-doc-files
(find $RPM_BUILD_ROOT% -type f | sed -e s@^$RPM_BUILD_ROOT@@) >> perl-files
%if %0
(find $RPM_BUILD_ROOT% $RPM_BUILD_ROOT/Documentation -type f | grep -vE "archimport|svn|git-cvs|email|gitk|git-gui|git-citool" | sed -e s@^$RPM_BUILD_ROOT@@ -e 's/$/*/' ) >> bin-man-doc-files
%else
rm -rf $RPM_BUILD_ROOT%
%endif
rm -rf $RPM_BUILD_ROOT%/locale
mkdir -p $RPM_BUILD_ROOT%/bash_completion.d
install -m 644 -T contrib/completion/git-completion.bash $RPM_BUILD_ROOT%/bash_completion.d/git
%clean
rm -rf $RPM_BUILD_ROOT
